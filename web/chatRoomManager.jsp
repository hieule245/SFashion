                ...
                <table class="table table-striped table-hover">
                    ...
                    <tbody id="listCustomer" class="align-middle">
                        <c:forEach items="${chatrooms}" var="item">
                            <tr id="item" data-id="${item.roomId}">
                                <c:if test="${sessionScope.user.getUsername() == item.user1.getUsername()}">
                                    <td class="align-middle">${item.user2.getUsername()}</td>
                                </c:if>
                                <c:if test="${sessionScope.user.getUsername() == item.user2.getUsername()}">
                                    <td class="align-middle">${item.user1.getUsername()}</td>
                                </c:if>
                                <td class="align-middle">${item.messages[item.messages.size() - 1].getContent()}</td>
                                <td class="align-middle">
                                    <form action="chatBoxManager" id="signup-form" class="btn">
                                        <input type="hidden" name="room_id" id="room_id" value="${item.roomId}"/>
                                        <div>
                                            <button type="submit" name="action" value="chat" class="btn btn-primary"><i class="material-icons" data-toggle="tooltip" title="Chat">&#xE254;</i></button>
                                        </div>
                                    </form></td>
                            </tr>
                        </c:forEach>
